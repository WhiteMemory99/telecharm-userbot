...
from app.utils.decorators import doc_args, USER_IDENTIFIERS, DURATION
...
async def kick(_, message: Message):
    ...
    if parse is None:
        ...
        if parse.is_self:
            await message.edit_text("Cannot kick myself.", message_ttl=conf.default_ttl)
        if parse.is_admin:
            await message.edit_text("Cannot kick admin.", message_ttl=conf.default_ttl)
        if not parse.is_member:
            ...
            try:
                ...
                await message.edit_text("Not enough rights to kick.", message_ttl=conf.default_ttl)
...
async def ban(client: Client, message: Message):
    ...
    The banned user will not be able to return to the group until they get unbanned or the duration time has reached.
    """
    ...
    if parse is None:
        ...
        if parse.is_self:
            await message.edit_text("Cannot ban myself.", message_ttl=conf.default_ttl)
        if parse.is_admin:
            await message.edit_text("Cannot ban admin.", message_ttl=conf.default_ttl)
        else:
            try:
                ...
                await message.edit_text("Not enough rights to ban.", message_ttl=conf.default_ttl)
            except RPCError as ex:
...
async def unban(_, message: Message):
    """Unban a previously banned group member by replying to their message or passing proper arguments."""
    parse = await parse_command(message, with_time=False)
    if parse is None:
        await message.edit_text("<b>Reply to message</b> or provide a valid <b>username/mention/ID</b>.", message_ttl=5)
    else:
        if parse.is_self:
            await message.edit_text("Cannot unban myself.", message_ttl=conf.default_ttl)
        if parse.is_admin:
            await message.edit_text("Admin could not be banned.", message_ttl=conf.default_ttl)
        if not parse.is_kicked:
            ...
            try:
                ...
                await message.edit_text("Not enough rights to unban.", message_ttl=conf.default_ttl)
            except RPCError as ex:
...
async def mute(_, message: Message):
    ...
    The muted user will not be able to send any messages until they get unmuted or the duration time has reached.
    """
    parse = await parse_command(message)
    if parse is None:
        await message.edit_text("<b>Reply to message</b> or provide a valid <b>username/mention/ID</b>.", message_ttl=5)
        ...
        if parse.is_self:
            await message.edit_text("Cannot mute myself.", message_ttl=conf.default_ttl)
        if parse.is_admin:
            await message.edit_text("Cannot mute admin.", message_ttl=conf.default_ttl)
        else:
            try:
                ...
                await message.edit_text("Not enough rights to mute.", message_ttl=conf.default_ttl)
            except RPCError as ex:
...
async def unmute(_, message: Message):
    """Unmute a previously muted group member by replying to their message or passing proper arguments."""
    parse = await parse_command(message, with_time=False)
    if parse is None:
        await message.edit_text("<b>Reply to message</b> or provide a valid <b>username/mention/ID</b>.", message_ttl=5)
    else:
        if parse.is_self:
            await message.edit_text("Cannot unmute myself.", message_ttl=conf.default_ttl)
        if parse.is_admin:
            await message.edit_text("Admin could not be muted.", message_ttl=conf.default_ttl)
        if not parse.is_restricted:
            ...
            try:
                ...
                await message.edit_text("Not enough rights to unmute.", message_ttl=conf.default_ttl)
            except RPCError as ex:
...
async def pin(client: Client, message: Message):
    ...
    if not message.reply_to_message:
        await message.edit_text("<b>Reply to the message</b> you want to pin.", message_ttl=conf.default_ttl)
    else:
        ...
        if full_chat.pinned_message and full_chat.pinned_message.message_id == message.reply_to_message.message_id:
            await message.edit_text("This message is already pinned.", message_ttl=conf.default_ttl)
        else:
            ...
            try:
                ...
                await message.edit_text("Not enough rights to pin.", message_ttl=conf.default_ttl)
                ...
                await message.edit_text("Failed to pin this message.", message_ttl=conf.default_ttl)
    ...
    if full_chat.pinned_message is not None:
        try:
            if "all" in message.get_args():
                ...
                await message.edit_text("Unpinned all the pinned messages.", message_ttl=conf.default_ttl)
            else:
                if message.reply_to_message:
                    await client.unpin_chat_message(message.chat.id, message.reply_to_message.message_id)
                    await message.edit_text("Unpinned this message.", message_ttl=conf.default_ttl)
                else:
                    await client.unpin_chat_message(message.chat.id, full_chat.pinned_message.message_id)
                    await message.edit_text("Unpinned the latest pinned message.", message_ttl=conf.default_ttl)
        except ChatAdminRequired:
            await message.edit_text("Not enough rights to unpin.", message_ttl=conf.default_ttl)
            ...
            await message.edit_text("Failed to unpin.", message_ttl=conf.default_ttl)
    else:
        await message.edit_text("There is no any pinned message here.", message_ttl=conf.default_ttl)
