...
async def kick(_, message: Message) -> None:
    """
    ...
    if parse is None:
        await message.edit_text(
            "<b>Reply to message</b> or provide a valid <b>username/mention/ID</b>."
        )
    else:
        if parse.is_self:
    ...
    if parse is None:
        ...
        if parse.is_admin:
            ...
            try:
                ...
                await message.edit_text(f"Banned {parse.user.mention} <b>{parse.text}</b>.")
...
@doc_args(USER_IDENTIFIERS)
async def unban(_, message: Message) -> None:
    """
...
@doc_args(USER_IDENTIFIERS, DURATION)
async def mute(_, message: Message) -> None:
    """
    ...
    if parse is None:
        ...
        if parse.is_admin:
            ...
            try:
                ...
                await message.edit_text(f"Muted {parse.user.mention} <b>{parse.text}</b>.")
            except (ChatAdminRequired, UserAdminInvalid):
...
@doc_args(USER_IDENTIFIERS)
async def unmute(_, message: Message) -> None:
    """
...
async def pin(client: Client, message: Message) -> None:
    ...
    if not message.reply_to_message:
        ...
        if (
            ...
            and full_chat.pinned_message.message_id == message.reply_to_message.id
        ):
            await message.edit_text("This message is already pinned.")
        else:
            if "loud" in message.command:
                status = "loudly"
                is_silent = False
            else:
                status = "silently"
            ...
            try:
                ...
                await message.edit_text(f"Pinned the message <b>{status}</b>.")
