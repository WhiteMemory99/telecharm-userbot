...
async def clean_up_switcher(client: Client, message: Message):
    """
    Turn on / off clean up mode that automatically deletes userbot messages some time after commands execution.
    ...
    await message.edit_text(f"Clean up is <b>{status}</b>.", message_ttl=conf.default_ttl)
...
async def autofwd_switcher(client: Client, message: Message):
    ...
    status = "off" if last_value else "on"
    await message.edit_text(f"Dot auto-forward is <b>{status}</b>.", message_ttl=conf.default_ttl)
